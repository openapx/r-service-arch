...
# -- arch service account

echo "-- arch service account"

adduser --system --group --shell /bin/bash --no-create-home --comment "arch service user account" --quiet arch

usermod -a -G vaultuser arch
...
echo "   - downloading arch service"

SOURCE_ASSET=$(curl -s -H "Accept: application/vnd.github+json" \
                    -H "X-GitHub-Api-Version: 2022-11-28" \
                    https://api.github.com/repos/openapx/r-service-arch/releases/latest )

SOURCE_URL=$( echo ${SOURCE_ASSET} | jq -r '.assets[] | select( .name | match( "^arch.service_\\d+.\\d+.\\d+.tar.gz$") ) | .browser_download_url' )
ARCH_SOURCE=$( echo ${SOURCE_ASSET} | jq -r '.assets[] | select( .name | match( "^arch.service_\\d+.\\d+.\\d+.tar.gz$") ) | .name' )

curl -sL -o /sources/R-packages/${ARCH_SOURCE} ${SOURCE_URL}


_MD5=($(md5sum /sources/R-packages/${ARCH_SOURCE}))
_SHA256=($(sha256sum /sources/R-packages/${ARCH_SOURCE}))

echo "      ${ARCH_SOURCE}   (MD5 ${_MD5} / SHA-256 ${_SHA256})"
...
Rscript -e "install.packages( c( \"sodium\", \"openssl\", \"plumber\", \"jsonlite\", \"pool\", \"DBI\", \"digest\", \"uuid\", \"httr2\"), type = \"source\", destdir = \"/sources/R-packages\" )" >> /logs/openapx/arch/install-r-packages.log 2>&1

echo "   - install R package cxapp"
Rscript -e "install.packages( \"/sources/R-packages/${CXAPP_SOURCE}\", type = \"source\", INSTALL_opts = \"--install-tests\" )" >> /logs/openapx/arch/install-r-packages.log 2>&1

echo "   - install R package arch service"
Rscript -e "install.packages( \"/sources/R-packages/${ARCH_SOURCE}\", type = \"source\", INSTALL_opts = \"--install-tests\" )" >> /logs/openapx/arch/install-r-packages.log 2>&1

echo "   - install R packages for PostgreSQL"
Rscript -e "install.packages( \"RPostgreSQL\", type = \"source\", destdir = \"/sources/R-packages\" )" >> /logs/openapx/arch/install-r-packages.log 2>&1
...
# -- set up example arch database

echo "   - create arch database"
...
#  disabled ... psql --quiet -d arch -f /opt/openapx/apps/arch/library/arch.service/db/postgres.sql

su postgres -c 'bash -s' <<EOF
psql --quiet -c "CREATE DATABASE arch WITH ENCODING=UTF8;"
psql --quiet -c "CREATE USER archsvc WITH ENCRYPTED PASSWORD '$(cat /.vault/dblocal/archsvc/password)';"
EOF
...
mkdir -p /data/arch/logs

chgrp -R arch /data/arch
chmod -R g+rws /data/arch
...
#    note: the archsvc password for the local database is generated when  
#          /entrypoint.sh is executed and local database is enabled
DB.USERNAME = archsvc
DB.PASSWORD = [vault] /dblocal/archsvc/password



# -- logging 
LOG.PATH = /data/arch/logs
LOG.NAME = arch
LOG.ROTATION = month
...
#    note: see reference section Authentication in the arch service API reference
...
API.AUTH.SECRETS = /api/auth/arch/services/* /api/auth/arch/users/*
